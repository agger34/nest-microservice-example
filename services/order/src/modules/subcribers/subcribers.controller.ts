...
export class SubcribersController {
  ...
  handlePaymentConfirmed(data: PaymentEventModel) {
    ...
    this.commandBus.execute(new UpdateOrderCommand(data.orderId, OrderStateConst.CONFIRMED));
  }
  ...
  handlePaymentDeclined(data: PaymentEventModel) {
    ...
    this.commandBus.execute(new UpdateOrderCommand(data.orderId, OrderStateConst.CANCELLED));
  }
}
